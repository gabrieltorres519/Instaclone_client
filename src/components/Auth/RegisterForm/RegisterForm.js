    ...
    export default function RegisterForm(props) {
        ...
        const formik = useFormik({
          ...
          //Aquí se usa Yup para que si no se cumplen las validaciones indicadas el onSubmit no se ejecute
          validationSchema: Yup.object({
            name: Yup.string().required("Tu nombre es obligatorio"),
            username: Yup.string().matches(/^[a-zA-Z0-9-]*$/,"El nombre de usuario no puede tener espaicio").required("El nombre de usuario es obligatorio"),
      ...
      return (
        <>
          ...
          <Form className="register-form" onSubmit={formik.handleSubmit}> 
            ...
            <Form.Input type="text" placeholder="Correo electrponico" name="email" value={formik.values.email} onChange={formik.handleChange} error={formik.errors.email && true} />
            <Form.Input type="password" placeholder="Contraseña" name="password" value={formik.values.password} onChange={formik.handleChange} error={formik.errors.password} />
            <Form.Input type="password" placeholder="Repetir contraseña" name="repeatPassword" value={formik.values.repeatPassword} onChange={formik.handleChange} error={formik.errors.repeatPassword} />
